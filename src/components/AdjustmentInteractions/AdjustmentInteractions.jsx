...
class AdjustmentInteractions extends Component {
    ...
    checkForSelection = (index, fullRes = false) => {

        if (!this.props.selectionInfo.selectionEnabled)
            return true;

        index = index / 4
        let selectionMapping = fullRes ? this.props.selectionInfo.fullResSelectionMapping : this.props.selectionInfo.selectionMapping;

        let i = parseInt(index / selectionMapping[0].length)
    ...
    applyChangesToFullResImage = () => {
        ...
        for (let i = 0; i < imageData.length; i += 4) {
            if (!this.checkForSelection(i, true))
                continue;
            this.changeBrightNess(imageData, i);
            this.changeContrast(imageData, i);
            this.changeSatruration(imageData, i);
